# 구미과외 (gumi-tutoring) — 작업 규칙

경상북도 구미시 지역 타겟 과외 사이트. 정적 사이트, 빌드 없음.
...
- **검색엔진 소유확인 태그** (`index.html`, 2026-09-22 손으로 넣음): `naver-site-verification` = `db66169e330617a3c974c25b9ca087d1486a1632` · `google-site-verification` = `VUaaMlapbDYNG_DLFnRDjIHSwFF5JFawfjRbjuXc4Q4`. 생성기(`regions.js`)에는 아직 없어서 **재생성하면 사라진다** — 재생성 후 이 태그를 다시 넣거나, 생성기 head 부분에 사이트별 인증값을 추가할 것.
...
| `schools.html` | 구미 초·중·고 103곳 목록 + 검색 |
| `school/*.html` | **학교별 정적 페이지 103개** — SEO 의 핵심. 학교당 국어·영어·수학·사회·과학·코딩 키워드 |
| `teachers.html` | 선생님 (구미 방문 2명 + 화상 673명) |
| `online.html` | 화상과외 안내 |
| `centers.html` | 구미 학습센터 1곳 |
...
5. `& "$env:USERPROFILE\사이트관리\로그기록.ps1" -Repo gumi-tutoring -Status 완료 -What "글 N편"`
